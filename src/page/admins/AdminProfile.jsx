...
const AdminProfile = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/admins/admininfo', {
        params: { id: id }
      });
  ...
  const handleOnSubmit = async (e) => {
    ...
    try {
      await axios.put("http://localhost:5000/api/admins/adminprofile", payload)
        .then(response => {
          if (response.data && response.data.status === "Success") {
            console.log(response.data.status);
            setData(data.map(admins => admins.id === id ? data : admins));
